...
function [mag_r, pha_r, w_r] = mybode (varargin)
  ...
  if (! nargout)
    ...
    subplot (2, 1, 1)
    semilogx (mag_args{:},"linewidth",2)
    axis ("tight")
    ylim (__axis_margin__ (ylim))
    grid ("on")
    ylabel ("$\left| G ( i \omega ) \right| [dB]")

    subplot (2, 1, 2)
    semilogx (pha_args{:},"linewidth",2)
    axis ("tight")
    ylim (__axis_margin__ (ylim))
    grid ("on")
    xlabel ("Frequency [rad/s]")
    ylabel ("$\angle G (i \omega ) [deg]")
    legend (legend_args)
  else
    mag_r = mag{1};
    pha_r = pha{1};
