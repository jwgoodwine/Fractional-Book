...
\begin{itemize}
  ...
  \begin{align*}
    ...
    \begin{equation}
      \boxed{ E_\alpha(t) = \sum_{k=0}^\infty \frac{t^k}{\Gamma \left( \alpha k + 1 \right)}, \quad \alpha > 0 }
      \label{eq:mlone}
    \end{equation}
    and
    \begin{equation}
      \boxed{  E_{\alpha, \beta}(t) = \sum_{k=0}^\infty \frac{t^k}{\Gamma \left( \alpha k + \beta \right)}, \quad \alpha, \beta > 0. }
      \label{eq:mltwo}
    \end{equation}
...
%}
    ...
    \begin{example}
      ...
      \begin{align*}
	\tensor*[^C]{\D}{^{1/2}} \left( t + 1 \right) &= \frac{1}{\Gamma \left( \frac{1}{2} \right) } \int_0^t \left( t - z \right)^{-\frac{1}{2}}
    ...
    Now we will consider calculus in the frequency domain. First, recall the basic property of Laplace transforms that the Laplace transform of convolution in the time domain is multiplication in the frequency domain, specifically
    \begin{align*}
    ...
    For fractional differentiation in the frequency domain, we will get a different answer for the Riemann-Liouville and Caputo definitions. 
...
\begin{align*}
  ...
  & \qquad - s^{\lc \alpha \rc - 2} \left[ \frac{\d}{\d t}\left( \tensor*[]{\D}{^{-(\lc \alpha \rc - \alpha)}}f(t) \right) \right]_{t=0} \\
  & \qquad - \cdots -  \left[ \frac{\d^{\lc \alpha \rc - 1}}{\d t^{\lc \alpha \rc - 1}}\left( \tensor*[]{\D}{^{-(\lc \alpha \rc - \alpha )}}f(t) \right) \right]_{t=0}. 
\end{align*}
...
\begin{align*}
  \mathcal L \left\{ \tensor*[^C]{\D}{^\alpha} f(t) \right\} &= \mathcal L \left\{ 
	...
	&= \frac{1}{\Gamma \left( \lc \alpha \rc - \alpha \right)} \mathcal L \left\{  t^{\lc \alpha \rc - \alpha - 1} \right\} \mathcal L \left\{  \frac{\d^{\lc \alpha \rc}f}{\d t^{\lc \alpha \rc}} (t) \right\} \\
